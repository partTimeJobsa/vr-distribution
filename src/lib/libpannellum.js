/* eslint-disable */
var libpannellum = (function(window, document, undefined) {
...
function Renderer(container) {
    ...
    this.init = function(_image, _imageType, _dynamic, haov, vaov, voffset, callback, params) {
     ...
     this.updateTexture = function(_image, _imageType) {
        if (_imageType == 'cubemap') {
            if(!Array.isArray(_image)) return false
            image = _image
            // Load all six sides of the cube map
            gl.texImage2D(gl.TEXTURE_CUBE_MAP_POSITIVE_X, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, _image[1]);
            gl.texImage2D(gl.TEXTURE_CUBE_MAP_NEGATIVE_X, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, _image[3]);
            gl.texImage2D(gl.TEXTURE_CUBE_MAP_POSITIVE_Y, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, _image[4]);
            gl.texImage2D(gl.TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, _image[5]);
            gl.texImage2D(gl.TEXTURE_CUBE_MAP_POSITIVE_Z, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, _image[0]);
            gl.texImage2D(gl.TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, _image[2]);
        } else {
            if (_image.width <= maxWidth) {
                gl.uniform1i(gl.getUniformLocation(program, 'u_splitImage'), 0);
                // Upload image to the texture
                gl.texImage2D(glBindType, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, _image);
            } else {
                // Image needs to be split into two parts due to texture size limits
                gl.uniform1i(gl.getUniformLocation(program, 'u_splitImage'), 1);
                ...
                // Draw image on canvas
                var cropCanvas = document.createElement('canvas');
                cropCanvas.width = _image.width / 2;
                cropCanvas.height = _image.height;
                var cropContext = cropCanvas.getContext('2d');
                cropContext.drawImage(_image, 0, 0);

                // Upload first half of image to the texture
                var cropImage = cropContext.getImageData(0, 0, _image.width / 2, _image.height);
                gl.texImage2D(glBindType, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, cropImage);
                ...
                // Upload second half of image to the texture
                cropContext.drawImage(_image, -_image.width / 2, 0);
                cropImage = cropContext.getImageData(0, 0, image.width / 2, _image.height);
                gl.texImage2D(glBindType, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, cropImage);
