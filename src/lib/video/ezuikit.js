      ...
      addJs(jsPluginPath, function() {
        ...
        _this.jSPlugin.JS_SetWindowControlCallback({
          ...
          pluginErrorHandler: function(iWndIndex, iErrorCode, oError) { // 插件错误回调
            ...
            if (playParams && playParams.handleError) {
              playParams.handleError({ retcode: iErrorCode, msg: oError || '播放失败，请重试' })
              if (playParams.url.indexOf('alarmId') !== -1) {
        ...
        _this.jSPlugin.JS_SetOptions({
          ...
          bOnlySupportJSDecoder: !!((typeof playParams.useMSE === 'undefined' || playParams.useMSE === false)) // 是否强制使用jsdecoder
        }).then(function() {
